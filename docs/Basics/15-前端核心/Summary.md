...
**<u>*2、从逻辑层面上看*</u>**：在调用函数，执行函数代码前，会有两个阶段：内存分配的创建阶段、以及代码执行阶段；
...
- **<u>*创建阶段—CreationPhase*</u>**：完成执行上下文/执行环境(执行代码时的运行环境)的创建，上下文的创建还包含以下对象的创建：
  ...
  - 2-2、变量对象 VO 的创建：此对象形成所有可访问变量，可理解为 **<u>作用域</u>** 实体，进入变量的生命周期(声明/初始化/定义)；
    - 创建实参对象 (arguments object)，检查 context 形参 (parameters)，初始化参数名和参数值，并创建一份引用拷贝；
    - 扫描 context 中的函数声明：

      - 为每一函数在 VO 上创建属性，属性名即函数名，含有一个指向内存中函数的引用指针，进行声明初始化赋值三连
...
- **<u>*执行阶段—ExecutionPhase*</u>**：执行上下文被推入 **<u>执行栈</u>**，运行/解释 执行上下文 中函数代码，并且根据代码一行一行的执行；
  - 2-1、变量对象的转换：由 VO 转为 AO，实际乃同一对象不同时期的称呼；执行时发生变量的赋值也即定义操作；
    ...
    - 若为 let 变量，则进入变量的初始化，后续若有别的赋值操作则进行赋值操作；
      - 注意：在初始化前，若使用 let 变量，即使用已声明但未初始化的变量，就会进入 TDZ 报错；xx is not defined
...
**<u>变量提升</u>**：变量对象的创建的衍生概念，即开始进入变量的生命周期的系列现象：声明(Declaration phase)、初始化(Initialization phase) 与赋值(Assignment phase)；
...
- 然后，代码直接执行过程中，若发现**新的宏任务** 交由事件触发线程管理(推入宏任务队列)，**微任务**进入引擎线程的微任务队列；
- 然后，当前宏任务执行完全，执行栈为空，检查微任务队列，若有则依次执行，直到微任务队列为空；若此时遇到新的宏任务，则将其再次移交事件触发线程管理，**<u>若遇到新的微任务，则将其推入新的微任务队列中，并在执行栈内容执行为空后再去执行；</u>**
- 然后，执行浏览器 UI 线程的渲染工作；
...
- 微任务：由JS引擎线程维护；当前宏任务执行结束后立即执行的任务，在 UI渲染前执行；
