...
- **<u>Expires(服务端最初返回此内容时，标记上的字段，表示过期时间—缺点明显—服务客户两端时间不一致)</u>**
- **<u>Cache-control 下的 max-age 键—客户端接收后开始计算，资源的最长存活时间；它还有很多 key !!!</u>**
  - **public**：客户端、中间代理服务器均允许缓存资源；
  - **private**：只有客户端能缓存，中间代理服务器不能缓存；
  - **no-cache**：跳过当前强缓存检查阶段，直接发送 HTTP 请求，<u>即直接进入协商缓存阶段；</u>
  - **no-store**：不进行任何形式的缓存；
...
**<u>*最后*</u>**，关于缓存位置，按优先级从高到低排列：Service Worker、Memory Cache、Disk Cache、Push Cache、网络请求：
...
<img src="https://leibnize-picbed.oss-cn-shenzhen.aliyuncs.com/img/20201010072318.png" alt="截屏2020-10-10 上午7.23.11" style="zoom:50%;" />

<img src="https://leibnize-picbed.oss-cn-shenzhen.aliyuncs.com/img/20201010072331.png" alt="截屏2020-10-10 上午7.23.25" style="zoom: 50%;" />
...
Cookie 本质是浏览器中存储的一个很小的、内容以键值对形式存储的文本文件，每次向同一域名发送请求，均会自动携带相同Cookie(这点被 CSRF 利用，可<u>设置 Samesite</u>：Strict 禁止随同发送；或使用 Token—毕竟不会自携带 Token)；服务器拿到 Cookie 后进行解析，即可获取客户端状态(Cookie 多用于存储用户状态，所以被 XSS 利用，<u>设置 Cookie</u>: HttpOnly 字段禁止 JS 读取)；即 Cookie 用于 <u>状态存储</u>，但缺点也很多：
...
**<u>IndexedDB</u>**：理论容量无上限，为大型数据的存储提供了接口；
...
请求发出前，浏览器会自动为请求头中，添加字段 `Origin`，用以说明请求来源；当服务器拿到请求并回应时，会相应地添加字段 `Access-Control-Allow-Origin`，并设置字段值(或请求的 Origin 值，或别值)；然后，当浏览器收到时，若发现 `Origin` 值不在此字段范围内时，就会将响应拦截；所以**<u>字段 `Access-Control-Allow-Origin` 是服务器用来决定浏览器是否拦截此响应的必需字段</u>**；
...
**<u>非简单跨域请求</u>**：浏览器则会先使用 `OPTIONS` 方法，<u>自动发起预检请求到服务器，以获知服务器是否允许该实际请求，避免跨域请求对服务器的用户数据产生未预期的影响</u>；主要体现在两方面：**<u>预检请求</u>**、**<u>响应字段</u>**，主流程是：
...
**<u>生成 HTTP 请求消息</u>**：输入网址、浏览器解析 URL、生成 HTTP 请求信息

![](https://leibnize-picbed.oss-cn-shenzhen.aliyuncs.com/img/20201009120623.png)

![](https://leibnize-picbed.oss-cn-shenzhen.aliyuncs.com/img/20201009120635.png)



**<u>向 DNS 服务器查询 Web 服务器的 IP 地址</u>**
...
**<u>委托协议栈发送消息</u>**：协议栈通过 TCP 协议收发数据的操作
...
<img src="https://leibnize-picbed.oss-cn-shenzhen.aliyuncs.com/img/20201009121006.png" style="zoom:67%;" />

**<u>连接服务器</u>**：浏览器调用 Socket.connect

*   在 TCP 模块处创建表示连接控制信息的头部
*   通过 TCP 头部中的发送方和接收方端口号找到要连接的套接字

<img src="https://leibnize-picbed.oss-cn-shenzhen.aliyuncs.com/img/20201009121000.png" style="zoom:67%;" />

**<u>收发数据</u>**：浏览器调用 Socket.write
...
* 接收 HTTP 响应消息
  ...
  <img src="https://leibnize-picbed.oss-cn-shenzhen.aliyuncs.com/img/20201009121009.png" style="zoom:67%;" />

**<u>断开管道并删除套接字</u>**：浏览器调用 Socket.close

* 数据发送完毕后断开连接

  <img src="https://leibnize-picbed.oss-cn-shenzhen.aliyuncs.com/img/20201009121030.png" style="zoom:67%;" />
...
<img src="https://leibnize-picbed.oss-cn-shenzhen.aliyuncs.com/img/20201009121053.png" style="zoom:67%;" />

<img src="https://leibnize-picbed.oss-cn-shenzhen.aliyuncs.com/img/20201009121056.png" style="zoom:67%;" />
...
<img src="https://leibnize-picbed.oss-cn-shenzhen.aliyuncs.com/img/20201009143630.png" alt="截屏2020-10-09 下午2.36.26" style="zoom:67%;" />
...
- **<u>解析1：浏览器无法识别 HTML，须先将 HTML内容转换为能被理解的 DOM 树(本质是一以 document 为根节点的多叉树)；</u>**
...
- **<u>解析2：渲染引擎将 CSS 样式表转化为浏览器可理解的 styleSheets，计算出 DOM 节点的样式；</u>**
...
- **<u>渲染4：合成线程发送绘制图块命令给浏览器进程，浏览器进程根据指令生成页面，并显示到显示器上；</u>**
...
- 添加 `will-change: tranform`：让渲染引擎为节点单独实现一图层；在变换发生时仅利用 **<u>合成线程</u>** 去处理这些变换而不牵扯主线程，提高渲染效率；
...
- 从文件大小：前略；
  - 减少文件数量：减少页面上引用文件数量，以减少 HTTP 连接数；
- 从 `script` 标签使用：前略；
- 从 CSS、HTML 的代码书写：前略；
  - 不要使用嵌套 tables、指定 table 和图片的大小；
  - 使用现代 CSS 的合法标签：使用现代 CSS 来减少标签和图片，使用合法标签避免浏览器解析 HTML 时做 error correction。
- 从需要下载的内容是否需要在首屏使用：前略；
  - 优化页面元素的加载顺序，加载与内容相关的内容的脚本和样式；
...
**<u>浏览器解析规则(同上面对的渲染阻塞问题)</u>**：

*   <u>CSS 阻塞渲染</u>：CSSOM 负责存储渲染信息，浏览器就须保证在合成渲染树前，CSSOM 是完备的，只有 CSSOM 和 DOM 解析完全结束，浏览器才会进入下一步渲染；所以会发生 CSS 阻塞渲染，表现为在 CSSOM 完备前，页面一直处于白屏状态，这就是为什么样式表放在 head 中，仅仅是为了更快解析 CSS，保证更快首次渲染(但会造成阻塞)；
    *   CSS 放在 head 中会阻塞页面渲染(页面渲染会等到 CSS 加载完成)
    *   CSS 阻塞 JS 执行(因 GUI 线程和 JS 线程互斥，因为 JS 有可能会操作 DOM)
    *   CSS 不阻塞外部脚本的加载(不阻塞 JS 的加载，但阻塞 JS 的执行，因浏览器会有预先扫描器)
*   <u>JS 阻塞页面</u>：JS 可操作 DOM 来修改 DOM 结构，可操作 CSSOM 来修改节点样式，这就导致了常规情况下浏览器在解析 HTML 时，一旦碰到 Script 脚本，就须立即停止 HTML 解析，也阻塞了其后的 CSS 解析，整个解析进程必须等待 JS 的执行完成才能继续；从性能角度上讲，这就是为什么将 script 放在页面底部；
    *   直接引入 JS 会阻塞页面的渲染(JS 线程和 GUI 线程互斥)
    *   异步加载的 JS不阻塞页面的解析(script 标签中添加 defer 属性)
    *   异步加载的 JS下载过程不阻塞页面的解析，下载完成后立即执行，执行过程会阻塞页面的解析(script 标签中添加 async 属性)
    *   JS 不阻塞资源的加载；JS 顺序执行，阻塞后续 JS 逻辑的执行
