...
方法1：在子类函数中增加：父类函数.apply/call(this, arguments/…arguments);

方法2：prototype模式：

- Cat.prototype = new Animal(); Cat.prototype.constructor = Cat;

- 注意：但凡对 prototype 的赋值操作，均需重新将 prototype.constructor 修正为原有类；因原有类.constructor属性默认调用prototype对象的 constructor 属性；
- 问题：会覆盖 Cat上 prototype 原有属性(因直接 Cat.prototype = new Animal();)、耗内存(需建立父类实例 new Animal())
- 改进：即方法3；

方法3：直接继承 prototype，因不变的属性都可直接写入 Animal.prototype，故可直接继承 prototype 实现：

- Cat.prototype = Animal.prototype;

- 问题：Cat.prototype 和 Animal.prototype 现在指向了同一个对象，那么任何对 Cat.prototype 的修改，都会反映到Animal.prototype；
- 改进：即方法4；

方法4：利用 <u>空对象</u> 作为中介，几乎不占内存，且修改 Cat 的 prototype 对象，也不会影响到 Animal 的 prototype 对象
