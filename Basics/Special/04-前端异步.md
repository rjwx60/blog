...
Rxjs 适用于复杂场景下的异步操作：WebSocket(天生字节流) 数据流处理、复杂应用逻辑(如有大量异步数据更新，数据间还存在相互依赖的关系的大型应用)；
...
基本资料：[Rxjs文档](https://www.learnrxjs.io/)、[Rxjs各类操作符](https://reactive.how/)

核心：并非繁多的操作符，而是其响应式思想；注意不要强行使用 Rxjs，需要根据相关情况来具体使用(团队、应用复杂度、测试、必要性、性能等)，而不要为了使用 rxjs 而去使用 rxjs

场景：不同数据源的更新需求(Wb与Ajax共用)、WebSocket(字节流) 数据流处理、复杂应用逻辑(数据层级：如有大量异步数据更新且数据间还存在相互依赖的关系的大型应用)、业务实体间有依赖关系的系统(实体层级)、富事件驱动的场景(要处理各种键盘事件、鼠标事件等)；

缺点：基于发布订阅模式，代码置于黑盒中，很难调试；

区别：promise 乃单次异步，rxjs 支持与时间维度有关的多次异步；
...
// ReplaySubject 二次订阅时，才输出，输出首次订阅时缓存的值
// 当 next 时，与普通 subject 行为类似，分属两个输出
