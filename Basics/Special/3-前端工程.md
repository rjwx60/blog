...
基本：[CommonJS](https://en.wikipedia.org/wiki/CommonJS) 规范中，每个 JS 文件均为独立的模块上下文 (module context)，除非是 global 顶层对象属性，否则不能访问内部定义变量 (不污染全局作用域)；
...
定义：模块使用 define 定义、使用 require 调用、模块异步加载，依赖先执行；
...
实现：RequireJs、curl 、Dojo 、Nodules；

```js
// 1、定义与暴露模块
define(id?, dependencies?, factory); 
// define(函数(关键字), 将要依赖的项的列表, 回调函数);
// 回调函数的参数顺序 == 数组中的依赖顺序，此相当于导入模块，而回调函数返回的值即你导出的值；
...
<img src="/Image/Basics/Special/Engineering/10.png" style="zoom:50%;" align="left"/>

其他：[AMD 不被 CommonJs 认可(笑)](https://github.com/seajs/seajs/issues/588)



##### 1-2-6-2、CMD

定义：模块使用 define 定义 & 调用、依赖就近，延迟执行，懒加载
...
- `exports.xxx = value`；
- module 变量代表当前模块，此变量为一对象，其 exports 属性是对外接口；
...
- 服务端与浏览器端通用；
- 浏览器端：
  - 目前需要使用 Babel 将 ES6 转译 ES5
  - 利用 Browserify 编译；

##### 1-3-4-2、说明

- 依赖需要提前声明好，import 在顶层；
...
<img src="/Image/Basics/Special/Engineering/16.png" style="zoom:50%;" align="left"/>
