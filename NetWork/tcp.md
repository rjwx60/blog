...
​	TCP 负责控制两端间的流量，也要负责监控整个网络的拥塞情况并加以管理，而TCP 协议向应用层提供不定长的字节流发送服务，就表明 TCP 协议先天性就有意愿占满网络中的带宽，当网络中许多 TCP 连接同时试图占满整个带宽时，有可能发生恶性<u>拥塞</u>事件，所以有必要启用拥塞控制，以减少拥塞情况，提升网络整体速度；
...
​	综述：发送方维持一个**<u>*拥塞窗口 cwnd (congestion window)*</u>** 的状态变量；其大小取决于网络的拥塞程度，且会动态变化；发送方让自身发送窗口等于拥塞窗口，而其控制原则是：若网络不拥塞则增大拥塞窗口以发送更多分组；若网络拥塞则减小拥塞窗口以减少注入网络的分组；
